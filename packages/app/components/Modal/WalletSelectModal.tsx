import React from 'react';

export interface WalletSelectModalProps {
  title: string;
  visible: boolean;
  activate: (onError?: (error: Error) => void) => Promise<void>;
  // type?: 'info' | 'error';
  onConfirm: { label: string; onClick: Function };
}
export const DefaultWalletSelectModalProps: WalletSelectModalProps = {
  title: 'Select a wallet',
  visible: false,
  activate: async () => {},
  // type: 'info',
  onConfirm: { label: '', onClick: () => {} },
};

const walletOptions = [
  'injected',
  'portis',
  'fortmatic',
  'authereum',
  'bitski',
];

export const WalletSelectModal: React.FC<WalletSelectModalProps> = ({
  title,
  activate,
  visible,
  onConfirm,
}) => {
  console.log(onConfirm, 'lol', visible);
  if (!visible) return <></>;
  return (
    <div
      className="fixed z-10 inset-0 overflow-y-auto"
      aria-labelledby="modal-title"
      role="dialog"
      aria-modal="true"
    >
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div
          className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
          aria-hidden="true"
        ></div>

        <span
          className="hidden sm:inline-block sm:align-middle sm:h-screen"
          aria-hidden="true"
        >
          &#8203;
        </span>

        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
          <div>
            <div className="mt-3 text-center sm:mt-5">
              <h3
                className="text-lg leading-6 font-medium text-gray-900"
                id="modal-title"
              >
                {title}
              </h3>
            </div>
          </div>
          <div className="flex z-30 justify-center items-center flex-col">
            {walletOptions.map((connector) => {
              return (
                <div className="w-12" onClick={() => activate()}>
                  <p className="text-center font-medium">{connector}</p>
                </div>
              );
            })}
          </div>
          <div className="mt-5 sm:mt-6">
            <button
              type="button"
              className="button button-primary w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm"
              onClick={() => {
                console.log(onConfirm.onClick, 'onclick');
                onConfirm.onClick && onConfirm.onClick();
              }}
            >
              {onConfirm.label}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};
export default WalletSelectModal;
